# job_hunter_iter — 复盘分析模块（Phase 0）

在 `octpus1230216-alt/job_hunter` 上加的「向后复盘闭环」最小可用实现。
目标：收集**已投递 / 已读不回 / 不合适**的结果，分析为啥没回音，对照岗位改简历，量化版本改进。

> 借鉴自 `MadsLorentzen/ai-job-search`（代码已逐文件读过）：`outcome.md` 数据模型、`04-job-evaluation.md` 五维评分、`apply.md` 的诚实不虚构原则、`job-scraper` 可插拔 portal 架构、`verify_pdf.py` 的 ATS 思路（Phase 2）。
...
python score.py --all        # 离线 heuristic 五维评分（无 key 可跑，默认取最新简历作 profile）
...
| `score.py` | 五维匹配度评分（技能/经验/文化/地点/职业，加权出总评） | 离线 heuristic（默认）/ LLM 直连（`--mode llm`）/ 导出提示词（`--export-prompt`） |
...
| `docs/DESIGN_SPEC.md` | 设计规格：架构、数据模型、状态机、五维评分、双通道、模块接口、A/B 口径 |
...
- ⏳ 待做：可插拔爬虫 connector（BOSS/脉脉 portal skill）、calibration loop（用复盘数据反哺评分框架）、Streamlit 界面、合并进 `job_hunter` 主仓
